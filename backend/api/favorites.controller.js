...
export default class FavoritesController {
    ...
    // add apiGetAllFavorites to get all favorite movies for a user
    static async apiGetAllFavorites(req, res, next){
        try{
            let id = req.params.userId;
            let favorites = await FavoritesDAO.getAllFavorites(id);
            if (!favorites){
                res.status(404).json({error:'not found'});
                return;
            }
